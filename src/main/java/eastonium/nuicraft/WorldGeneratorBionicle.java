...
public class WorldGeneratorBionicle implements IWorldGenerator {
	...
	private void generateOres(World world, Random random, int chunkX, int chunkZ){
		for(int i = 0; i < 15; i++)
		{
			...
			(new WorldGenMinable(Bionicle.ProtodermisOre.getDefaultState(), 4)).generate(world, random, new BlockPos(xCoord, yCoord, zCoord));
		}
		for(int j = 0; j < 8; j++)
		{
			int xCoord = chunkX + random.nextInt(16);
			...
			int zCoord = chunkZ + random.nextInt(16);
			(new WorldGenMinable(Bionicle.LightstoneOre.getDefaultState(), 8)).generate(world, random, new BlockPos(xCoord, yCoord, zCoord));
		}
		for(int j = 0; j < 2; j++)
		{
			int xCoord = chunkX + random.nextInt(16);
			...
			int zCoord = chunkZ + random.nextInt(16);
			(new WorldGenMinable(Bionicle.HeatstoneOre.getDefaultState(), 3)).generate(world, random, new BlockPos(xCoord, yCoord, zCoord));
		}
	}
	...
	private void generateReeds(World worldIn, Random rand, BlockPos position){
        //for (int i = 0; i < 20; ++i){
            ...
            //if (f != 50){
            	...
            	for (int k = 0; k < j; ++k){
            		if (Bionicle.Bamboo.canPlaceBlockAt(worldIn, blockpos)){
            			worldIn.setBlockState(blockpos.up(k), Bionicle.Bamboo.getDefaultState(), 2);
            		}
            	}
            //}
        //}
